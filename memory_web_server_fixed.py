...
- Per-request MemoryStore instances with proper cleanup (Nov 20, 2025)
  * Previously used global singleton that held embedded Qdrant lock forever
  * Now creates new instance per request via FastAPI Depends pattern
  * Each instance properly shutdown after request completes
  * Enables concurrent access: web server + MCP server + CLI tools simultaneously
  * Fixes embedded Qdrant "storage folder already accessed" error
"""
